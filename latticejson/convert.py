...
LATTICEJSON_MADX_MAP: Tuple[Tuple[str, Tuple[str, ...]], ...] = (
    ("Drift", ("DRIFT")),
    ("Dipole", ("SBEND", "RBEND")),
...
JSON_TO_MADX: Dict[str, str] = {x: y[0] for x, y in LATTICEJSON_MADX_MAP}
MADX_TO_JSON: Dict[str, str] = {y: x for x, tup in LATTICEJSON_MADX_MAP for y in tup}


def convert_to_latticejson(other_dict: dict, name_map: dict):
    """Convert a elegant/madx dict into a LatticeJSON dict.

    :param dict other_dict: dict with elegant/madx names
    ...
    elements = {}
    for name, (other_type, other_attributes) in other_dict["elements"].items():
        latticejson_type = name_map.get(other_type)
        if latticejson_type is None:
            elements[name] = ["Drift", {"length": other_attributes.get("L", 0)}]
            warnings.warn(
                f"Replacing element {name} ({other_type}) with Drift.", stacklevel=2
            )
            continue
        ...
        for other_key, value in other_attributes.items():
            ...
            if latticejson_key is not None:
                ...
                warnings.warn(
                    f"Ignoring attribute {other_key} of {name}.", stacklevel=2
                )

    lattices = other_dict["lattices"]
...
def elegant_to_latticejson(string):
    ...
    return convert_to_latticejson(elegant_dict, ELE_TO_JSON,)


def madx_to_latticejson(string):
    """Convert an madx lattice file to a LatticeJSON dict.
    ...
    return convert_to_latticejson(madx_dict, MADX_TO_JSON)
...
def latticejson_to_elegant(lattice_dict) -> str:
    ...
    strings = [f"! TITLE: {lattice_dict['name']}"]
    ...
    strings.append(lattice_template("__MAIN__", ", ".join(lattice_dict["lattice"])))
